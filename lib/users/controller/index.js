  ...
  try {
    ...
    if (user) {
      ...
      if (token) {
        ...
        const result = transformers.userTransformer({...user.dataValues, ...userInfo.dataValues})
    ...
    return responses.failedWithMessage("Please ckeck username and password ", res);
  } catch (err) {
    console.log("ERROR-->", err);
    return responses.serverError(res);
  }
